...
# 原来的 CONFIG_ENV 改为保留 TARGET_CFLAGS
LIBCEDARC_CONF_ENV = \
...
# -------------------
# 自定义 build 命令，确保 -Wno-error=use-after-free 生效
# -------------------
define LIBCEDARC_BUILD_CMDS
	@echo "Building libcedarc with CFLAGS: $(TARGET_CFLAGS) -Wno-error=use-after-free -Wno-error=stringop-truncation"
	$(MAKE) -C $(@D) \
		CFLAGS="$(TARGET_CFLAGS) -Wno-error=use-after-free -Wno-error=stringop-truncation" \
		LDFLAGS="$(TARGET_LDFLAGS) -L$(@D)/library/$(LIBCEDARC_ARCHLIB)" \
		all
endef
